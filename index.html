...
def pick_best_http_url(info: dict):
    """
    Cari format audio HTTP biasa (bukan HLS/DASH).
    Return (url, ext, abr) atau (None, None, None).
    """
...
async def get_stream_url(url: str = Query(...)):
    ...
    try:
        ...
        if not stream_url:
            raise HTTPException(
                status_code=500, detail="Stream URL tidak ditemukan"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Gagal mengambil stream URL: {str(e)}"
        )
...
async def proxy_audio(url: str = Query(...)):
    ...
    try:
            ...
            async def stream_direct():
                async with httpx.AsyncClient(
                    ...
                    async with client.stream(
                        "GET", http_url, headers=SC_HEADERS
                    ) as r:
                        if r.status_code >= 400:
                            raise HTTPException(
                                status_code=502,
                                detail=f"Upstream error {r.status_code}",
                            )
        ...
        # ── Fallback: HLS / apapun → download via yt-dlp ke tmpdir ──
        stream_url = pick_stream_url(info)
        if not stream_url:
            raise HTTPException(
                status_code=500, detail="Stream URL tidak ditemukan"
        ...
        if not hls:
            # Stream biasa yang tidak terdeteksi sebelumnya
            async def stream_fallback():
                async with httpx.AsyncClient(
                    timeout=120, follow_redirects=True
                ) as client:
                    async with client.stream(
                        "GET", stream_url, headers=SC_HEADERS
                    ) as r:
                        async for chunk in r.aiter_bytes(chunk_size=65536):
                            yield chunk
        ...
        # ── HLS: download dulu pakai yt-dlp (+ ffmpeg kalau ada) ──
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if not mp3_path or not os.path.exists(mp3_path):
                raise HTTPException(
                    status_code=500,
                    detail="File audio tidak ditemukan setelah download",
                )
